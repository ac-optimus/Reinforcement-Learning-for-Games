...
class DQNSolver:
    ...
    def experience_replay(self):
        ...
        for state, action, reward, state_next, terminal in batch:
            ...
            if not terminal:
                with torch.no_grad():
                    preds = self.q_network(torch.tensor(state_next))
                    update_q = torch.max(preds[0]).item()
                    if self.agent == "DDQN":
                        preds = self.q_traget(torch.tensor(state_next))
                        action = torch.argmax(preds[0]).item()
                        pred2 = self.q_network(torch.tensor(state_next))
                        update_q = pred2[0][action]
                    q_update = (reward + GAMMA * update_q)
            
            q_values_output = self.q_traget(torch.tensor(state))
